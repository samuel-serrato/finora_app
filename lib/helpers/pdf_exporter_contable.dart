...
class PDFExportHelperContable {
  ...
  final String? nombreUsuario; // <--- 1. NUEVA VARIABLE
  ...
  PDFExportHelperContable(
    ...
    this.context,
    this.nombreUsuario, // <--- 2. AGREGAR AL CONSTRUCTOR (Opcional)
  );
      ...
      children: [
        pw.Row(
          ...
          children: [
            ...
            pw.Text(
              'Programado: ${_formatDateSafe(pagoficha.fechasPago)}',
              style: const pw.TextStyle(fontSize: 5, color: PdfColors.grey600),
            ),
          ],
        ),
        ...
        if (pagoficha.saldofavor > 0) _buildSaldoFavorSummaryCardPdf(pagoficha),
        pw.SizedBox(height: 4),
        pw.Container(
                  ...
                  pw.Padding(
                    ...
                    child: pw.Row(
                      children: [
                        ...
                        if (isGarantia && isSaldoGlobal) pw.SizedBox(width: 2),
                        if (isSaldoGlobal)
  ...
  pw.Widget _buildTotalesCard() {
    ...
    final double totalPagofichaSinGarantia = reporteData.listaGrupos.fold(0.0, (
      sum,
      g,
    ) {
    ...
    final double totalFicha = reporteData.listaGrupos.fold(
      0.0,
            ...
            child: pw.Wrap(
              ...
              children: [
                ...
                _buildTotalItem(
                  ...
                  totalPagofichaSinGarantia,
                  secondaryText:
                      'Con Garantía: ${currencyFormat.format(totalPagofichaConGarantia)}',
                ),
                _buildTotalItem(
                  'Cap. Rec. (Período)',
                  totalCapitalRecaudadoSinGarantia,
                  secondaryText:
                      'Con Garantía: ${currencyFormat.format(totalCapitalRecaudadoConGarantia)}',
                ),
                _buildTotalItem(
                  'Interés Rec. (Período)',
                  totalInteresRecaudadoSinGarantia,
                  secondaryText:
                      'Con Garantía: ${currencyFormat.format(totalInteresRecaudadoConGarantia)}',
                ),
                _buildTotalItem(
                  'S. Favor Disp.',
                  reporteData.totalSaldoDisponible,
        ...
        pw.Expanded(
          ...
          child: pw.Row(
            ...
            children: [
              ...
              pw.SizedBox(width: 8),
              pw.Text(
                'P. Pago: ${grupo.pagoPeriodo}', // Abreviado
                style: pw.TextStyle(fontSize: 6),
              ),
            ],
          ),
  ...
  pw.TableRow _buildTotalesRow(List<Cliente> clientes) {
    final totalCapital = clientes.fold<double>(
      0,
      (sum, item) => sum + item.periodoCapital,
    );
    final totalInteres = clientes.fold<double>(
      0,
      (sum, item) => sum + item.periodoInteres,
    );
    final totalGeneral = clientes.fold<double>(
      0,
  ...
  pw.Widget _buildSimpleFinancialRow(
    String label,
    double value, {
    double fontSize = 7.0,
  }) {
